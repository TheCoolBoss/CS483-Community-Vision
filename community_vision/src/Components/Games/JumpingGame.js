...
const JumpingGame = forwardRef((props, ref) => {
    ...
    document.onkeydown = function (evt) {
        ...
        if (evt.keyCode === 32) {
            ...
            if (startScreen) {
                ...
                setScore(0);
                setEndScreen(false);
                setLetters([{ letter: getRandomLetter(), height: -10, x: 100 }]);
                setNewSpawn(0);
                interval = setInterval(() => {
                    gameTick();
                }, 20);
    ...
    var [letters, setLetters] = useState([{ letter: getRandomLetter(), height: -10, x: getRandomX() }]);
    var [startScreen, setStartScreen] = useState(true);
    ...
    function gameTick() {
        ...
        setLetters(currLetters => {
            for (var j = 0; j < currLetters.length; j++) {
                currLetters[j]['height'] = currLetters[j]['height'] + 0.10;
            }
            return [...currLetters];
        });
    }
    ...
    function addLetter() {
        setLetters(currLetters => {
            ...
            temp.push({ letter: getRandomLetter(), height: -10, x: getRandomX() });
            return temp;
        });
    }
    ...
    for (var j = 0; j < letters.length; j++) {
        ...
        if (tempLetters[j].letter === output) {
            if (tempLetters.length > 1) {
                ...
                setNewSpawn(0);
                tempLetters = [{ letter: getRandomLetter(), height: -10, x: getRandomX() }];
            }
                                        ...
                                        <button style={{ fontSize: '8vh', cursor: ' pointer', height: '100%', width: '100%' }}
                                            onMouseDown={function () {
                                                if (endScreen) {
                                                    ...
                                                    setScore(0);
                                                    setEndScreen(false);
                                                    setLetters([{ letter: getRandomLetter(), height: 0, x: getRandomX() }]);
                                                    setNewSpawn(0);
                                                    interval = setInterval(() => {
                                                        gameTick();
                                                    }, 20);
